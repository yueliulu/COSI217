...
class SpacyDocument:
    ...
    def get_entities_dependency(self):
        entities_dependency = dict()
        for e in self.doc.ents:
            for token in e:
                if e.text in entities_dependency:
                    entities_dependency[e.text].append((token.dep_, token.head.text, token.text))
                else:
                    entities_dependency[e.text] = []
                    entities_dependency[e.text].append((token.dep_, token.head.text, token.text))
                print(entities_dependency)
        return entities_dependency
